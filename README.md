...
Health Compass employs a sophisticated, local-first architecture centered around a "Planner-Agent" model, powered entirely by local Large Language Models (LLMs) via Ollama. This design ensures both privacy and functionality, allowing the application to understand user intent and take appropriate actions without data ever leaving the user's machine.

### High-Level AI-Powered Workflow

The core of the application's intelligence lies in its two-step process for handling user queries. This approach is heavily inspired by agentic AI patterns, where a "Planner" model first decides *what to do*, and then the application executes that plan.

1.  **Intent Resolution (The "Planner")**: When a user submits a query, it is first passed to a Planner LLM running on **Ollama**. This planner's sole responsibility is to analyze the query (along with conversation history) and determine the user's intent. It outputs a structured JSON object containing the `intent` and any relevant `entities` (e.g., dates, metric types, values). This step is crucial for transforming unstructured natural language into a machine-readable action plan.

2.  **Intent Handling (The "Agent")**: The application's backend receives this JSON object and uses it to route the request to the correct handler function.
...
```mermaid
graph TD
    A[User Enters Query] --> B{Planner LLM via Ollama};
    B -- "1. Intent Resolution" --> C[Structured JSON: Intent & Entities];
    C --> D{Application Router};

    subgraph "Path 1: Direct Answer"
        D -- "Intent: DIRECT_LLM_RESPONSE" --> E[LLM generates answer during planning];
    end

    subgraph "Path 2: Tool/Function Call"
        D -- "Intent: LOG_HEALTH_METRIC, QUERY_METRICS, EXPLAIN_PDF_REPORT" --> F[Execute Intent Handler Function];
        F --> G[Perform Action<br/>(DB Query, PDF Read, etc.)];
        G --> H[Generate Final Response<br/>(sometimes with another Ollama call)];
    end

    E --> Z[Display Response to User];
    H --> Z;
```
...
graph TD
    subgraph "Renderer Process (Frontend - React UI)"
        A[User Interface<br/>(App.jsx)] -- "1. User sends a message" --> B[IPC Call via Secure Bridge<br/>(window.electronAPI.invoke(...))];
    end

    subgraph "Main Process (Backend - Node.js)"
        C[IPC Handler<br/>(ipcMain.handle(...))] -- "3. Triggers Agent Logic" --> D[Planner/Agent Logic<br/>(Ollama, SQLite, PDF tools)];
        D -- "4. Generates response" --> C;
    end

    B -- "2. Crosses process boundary" --> C;
    C -- "5. Sends response back" --> B;
    B -- "6. Updates React state" --> A[User Interface Rerenders];
```
